...
fn read_chrome(start_ts: i64, end_ts: i64) -> Result<Vec<HistoryEntry>, String> {
    let src = chrome_history_path().ok_or("Chrome profile not found")?;
    let tmp = copy_db(&src, "chrome")?;

    let conn = Connection::open_with_flags(
        &tmp,
        OpenFlags::SQLITE_OPEN_READ_ONLY | OpenFlags::SQLITE_OPEN_NO_MUTEX,
    )
    .map_err(|e| format!("Open Chrome db: {e}"))?;

    // Chrome stores last_visit_time as microseconds since 1601-01-01 (Windows FILETIME)
    const FILETIME_TO_UNIX_US: i64 = 11_644_473_600 * 1_000_000;
    let start_chrome = start_ts * 1_000_000 + FILETIME_TO_UNIX_US;
    let end_chrome = end_ts * 1_000_000 + FILETIME_TO_UNIX_US;
    ...
    let mut stmt = conn
        ...
        .map_err(|e| format!("Prepare Chrome query: {e}"))?;

    let entries: Vec<HistoryEntry> = stmt
        .query_map([start_chrome, end_chrome], |row| {
            let visit_chrome: i64 = row.get(3)?;
            let url: String = row.get(1)?;
            let domain = extract_domain(&url);
            Ok(HistoryEntry {
                id: format!("ch_{}", row.get::<_, i64>(0)?),
                url,
                title: row.get(2)?,
                visit_time: (visit_chrome - FILETIME_TO_UNIX_US) / 1_000_000,
                browser: "chrome".to_string(),
                domain,
            })
        })
        .map_err(|e| format!("Chrome query: {e}"))?
        .filter_map(|r| r.ok())
        .collect();
...
fn get_history(
    ...
    if browsers.contains(&"chrome".to_string()) {
        match read_chrome(start_ts, end_ts) {
            Ok(mut entries) => all.append(&mut entries),
